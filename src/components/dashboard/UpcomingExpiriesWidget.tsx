...
export const UpcomingExpiriesWidget: React.FC<UpcomingExpiriesWidgetProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800/80">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="self-start sm:self-center text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1 transition-colors"
          >
            ...
            {expiringItems.length > 3 && (
              <div className="relative sm:w-48">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-blue-500"
                />
              </div>
            )}
          </div>
      ...
      {/* Expiries List */}
      <div className="divide-y divide-slate-100 dark:divide-slate-800/80 max-h-[440px] overflow-y-auto">
        {filteredItems.length === 0 ? (
          <div className="p-8 text-center space-y-2.5">
            <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto border border-emerald-200/60 dark:border-emerald-800/60">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
              {expiringItems.length === 0
                ? 'No library students have plans expiring in the next 7 days. Your active seat occupancy is steady.'
          ...
          filteredItems.map((item) => {
            ...
            if (daysLeft === 0) {
              urgencyBadgeClass = 'bg-rose-100 dark:bg-rose-950/70 text-rose-800 dark:text-rose-300 border-rose-200 dark:border-rose-800 animate-pulse font-bold';
              urgencyLabel = 'Expires Today';
            } else if (daysLeft === 1) {
              urgencyBadgeClass = 'bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-bold';
              urgencyLabel = 'Expires Tomorrow';
            } else if (daysLeft < 0) {
              urgencyBadgeClass = 'bg-rose-100 dark:bg-rose-950/70 text-rose-800 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-semibold';
              urgencyLabel = `Expired (${Math.abs(daysLeft)}d ago)`;
            } else if (daysLeft <= 3) {
              urgencyBadgeClass = 'bg-amber-100 dark:bg-amber-950/70 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800 font-bold';
              urgencyLabel = `In ${daysLeft} days`;
            } else {
              urgencyBadgeClass = 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800 font-medium';
              urgencyLabel = `In ${daysLeft} days`;
            }
            ...
            return (
              <div
                ...
                className="p-3.5 sm:p-4 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-pointer group"
              >
                {/* Left: Student info & badges */}
                <div className="flex items-start gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-slate-200 dark:group-hover:bg-slate-700 transition-colors">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-1 flex-wrap">
                      {seat && (
                        <span className="inline-flex items-center gap-1 font-mono font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-[11px]">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      {hasDue ? (
                        <span className="inline-flex items-center gap-0.5 text-[11px] font-bold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/60 border border-amber-200/80 dark:border-amber-800 px-1.5 py-0.2 rounded">
                          ₹{membership.outstandingAmount?.toLocaleString('en-IN')} Due
                        </span>
                      ) : (
                        <span className="inline-flex items-center text-[10px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.2 rounded">
                          Fees Cleared
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center shrink-0 pt-1 sm:pt-0">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs group/btn"
                  ...
                  <a
                    ...
                    className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors shadow-2xs"
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors"
